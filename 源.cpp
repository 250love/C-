...
// ============================================================================
int main() {
    cout << "=== C++ Compiler Frontend with Quaternion Generation and DAG Optimization ===" << endl;
    ...
    try {
        ...
        for (size_t i = 0; i < originalQt.size(); ++i) {
            ...
            // 将优化后的基本块合并为一个函数块
            vector<Quaternion> mergedQt;
            for (size_t j = 0; j < optimizedBlocks.size(); ++j) {
                for (size_t k = 0; k < optimizedBlocks[j].size(); ++k) {
                    mergedQt.push_back(optimizedBlocks[j][k]);
                }
            }
