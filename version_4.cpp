...
// ============================================================================
// ============================================================================
// SYMBOL class (Global Symbol Table Manager) - 完整修改版
// ============================================================================
class SYMBOL {
public:
    ...
    bool isTypeCompatible(const string& expected, const string& actual) {
        ...
        if (expectedBase == "double" && (actualBase == "int" || actualBase == "float")) return true;
    ...
    string getVariableType(const Token& token) {
        ...
        // 检查全局变量（如果有全局作用域表）
        if (!allList.empty()) {
            SecTable* globalTable = allList[0];
            map<string, Variable>::iterator it = globalTable->variableDict.find(varName);
            if (it != globalTable->variableDict.end()) {
                return it->second.fullType;
            }
        }
